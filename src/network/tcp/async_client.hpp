...
public:
    ...
    void Write(char *buff, int size, int timeout = 0) {
        if (timeout != 0) {
            ...
            deadline_.async_wait(std::bind(&AsyncClient::check_deadline, this, error_info));
        }
...
private:
    ...
    void start_connect(tcp::resolver::iterator endpoint_iter)
    {
        if (endpoint_iter != tcp::resolver::iterator())
        {
            ...
            socket_.async_connect(endpoint_iter->endpoint(),
                                  std::bind(&AsyncClient::handle_connect,
    ...
    void check_deadline(const std::string &error_info)
    {
        if (stopped_)
            return;
